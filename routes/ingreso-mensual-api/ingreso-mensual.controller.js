...
exports.deleteIngresoMensual = function (req,res){
	ingresoMensual.findByIdAndRemove(req.params.solicitudId,function(error){
		console.log('DELETE/ingresoMensual');
		if(error) return res.status(500).send(error.mensaje);
		res.status(200).send();
	});
};
